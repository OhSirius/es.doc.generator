...
namespace FSLib.Declension
{
    ...
    public static class Declension1251
    {
        ...
        private static int _mMaxResultStringBufSize = 500;
        /// <summary>
        ...
        public static int MaxResultBufSize
        {
            get
            {
                return _mMaxResultStringBufSize;
            }
            set
            {
                if(value < 1) 
                ...
                _mMaxResultStringBufSize = value;
            }
        }
            ...
            out string surname, out string name, out string patronimic)
        {
            ...
            try
            {
                parts = new FIOParts(_mMaxResultStringBufSize);
                ptr = StringToIntPtr(surnameNamePatronimic);
        ...
        {
            StringBuilder sb = new StringBuilder(_mMaxResultStringBufSize);
            int tmp = _mMaxResultStringBufSize;
        ...
        private static void CheckGender(Gender gender)
        {
            if(!Enum.IsDefined(typeof(Gender), gender) || gender == Gender.NotDefind)
                throw new ArgumentException(string.Format("Недопустимое значение рода: {0}", gender), "gender");
        }
        ...
        private static void CheckDeclensionCase(DeclensionCase declensionCase)
        {
            if(!Enum.IsDefined(typeof(DeclensionCase), declensionCase) || 
                ...
                throw new ArgumentException(string.Format("Недопустимое значение падежа: {0}", declensionCase), "declensionCase");
        }

        private static void FreeIntPtr(IEnumerable<IntPtr> ptrs)
        {
        ...
        private static string IntPtrToString(IList<IntPtr> ptrs, int resultLen)
        {
            return IntPtrToString(ptrs[ptrs.Count - 1], resultLen);
        }
        ...
        private static IntPtr[] StringsToIntPtrArray(params string[] strs)
        {
            ...
            ptrs[ptrs.Length - 1] = Marshal.AllocHGlobal(_mMaxResultStringBufSize);
        ...
        {
            public readonly IntPtr pSurname;
            public readonly IntPtr pName;
            public readonly IntPtr pPatronimic;
            public readonly Int32 lenSurname;
            public readonly Int32 lenName;
            public readonly Int32 lenPatronimic;
